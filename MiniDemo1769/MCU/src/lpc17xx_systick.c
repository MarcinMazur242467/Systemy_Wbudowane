...
void SYSTICK_InternalInit(uint32_t time)
{
	...
	maxtime = (float) (1 << 24) / (float) (SystemCoreClock / 1000) ;
	if(time > maxtime)
		//Error loop
		while(1);
	else
...
void SYSTICK_ExternalInit(uint32_t freq, uint32_t time)
{
	 ...
	 */
	//check time value is available or not
	maxtime = (float)(1 << 24) / (float)(freq / 1000) ;
	if (time > maxtime)
		//Error Loop
		while(1);
	else
	{
		 ...
		 * with time base is millisecond
		 */
		maxtime = (float)(((float)(freq) / 1000.0) * (float)time - 1.0);
