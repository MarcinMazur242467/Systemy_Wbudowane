...
FlagStatus TIM_GetIntStatus(LPC_TIM_TypeDef *TIMx, TIM_INT_TYPE IntFlag)
{
	...
	temp = (TIMx->IR) & (uint8_t)TIM_IR_CLR(IntFlag);
	if (temp)
		return (SET);
...
FlagStatus TIM_GetIntCaptureStatus(LPC_TIM_TypeDef *TIMx, TIM_INT_TYPE IntFlag)
{
	...
	CHECK_PARAM(PARAM_TIMx(TIMx));
	CHECK_PARAM(PARAM_TIM_INT_TYPE(IntFlag));
	temp = (TIMx->IR) & (uint8_t)(1 << (4 + IntFlag));
	if(temp)
		return (SET);
	return (RESET);
}
...
void TIM_ClearIntPending(LPC_TIM_TypeDef *TIMx, TIM_INT_TYPE IntFlag)
{
	CHECK_PARAM(PARAM_TIMx(TIMx));
	CHECK_PARAM(PARAM_TIM_INT_TYPE(IntFlag));
	TIMx->IR = (uint32_t)TIM_IR_CLR(IntFlag);
}
...
void TIM_Init(LPC_TIM_TypeDef *TIMx, TIM_MODE_OPT TimerCounterMode, void *TIM_ConfigStruct)
{
	...
	TIMx->CCR &= (uint8_t)(~TIM_CTCR_MODE_MASK);
	TIMx->CCR |= TIM_TIMER_MODE;

	TIMx->TC =0;
	TIMx->PC =0;
	TIMx->PR =0;
	TIMx->TCR |= (uint8_t)(1 << 1); //Reset Counter
	TIMx->TCR &= (uint8_t)(~(1 << 1)); //release reset
	if (TimerCounterMode == TIM_TIMER_MODE )
	{
	...
	else
	{
		...
		TIMx->CCR  &= (uint8_t)(~TIM_CTCR_INPUT_MASK);
...
void TIM_ConfigMatch(LPC_TIM_TypeDef *TIMx, TIM_MATCHCFG_Type *TIM_MatchConfigStruct)
{
	...
	if (TIM_MatchConfigStruct->IntOnMatch)
		TIMx->MCR |= (uint32_t)TIM_INT_ON_MATCH(TIM_MatchConfigStruct->MatchChannel);

	//reset on MRn
	if (TIM_MatchConfigStruct->ResetOnMatch)
		TIMx->MCR |= (uint32_t)TIM_RESET_ON_MATCH(TIM_MatchConfigStruct->MatchChannel);

	//stop on MRn
	if (TIM_MatchConfigStruct->StopOnMatch)
		TIMx->MCR |= (uint32_t)TIM_STOP_ON_MATCH(TIM_MatchConfigStruct->MatchChannel);

	// match output type

	TIMx->EMR   &= (uint32_t)(~TIM_EM_MASK(TIM_MatchConfigStruct->MatchChannel));
	TIMx->EMR   |= (uint32_t)(TIM_EM_SET(TIM_MatchConfigStruct->MatchChannel,TIM_MatchConfigStruct->ExtMatchOutputType));
}
/*********************************************************************//**
...
void TIM_ConfigCapture(LPC_TIM_TypeDef *TIMx, TIM_CAPTURECFG_Type *TIM_CaptureConfigStruct)
{
	...
	CHECK_PARAM(PARAM_TIMx(TIMx));
	TIMx->CCR &= (uint32_t)(~TIM_CCR_CHANNEL_MASKBIT(TIM_CaptureConfigStruct->CaptureChannel));

	if (TIM_CaptureConfigStruct->RisingEdge)
		TIMx->CCR |= (uint32_t)(TIM_CAP_RISING(TIM_CaptureConfigStruct->CaptureChannel));

	if (TIM_CaptureConfigStruct->FallingEdge)
		TIMx->CCR |= (uint32_t)(TIM_CAP_FALLING(TIM_CaptureConfigStruct->CaptureChannel));

	if (TIM_CaptureConfigStruct->IntOnCaption)
		TIMx->CCR |= (uint32_t)(TIM_INT_ON_CAP(TIM_CaptureConfigStruct->CaptureChannel));
}
