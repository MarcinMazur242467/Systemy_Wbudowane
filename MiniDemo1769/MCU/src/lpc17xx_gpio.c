...
void FIO_HalfWordSetDir(uint8_t portNum, uint8_t halfwordNum, uint16_t bitValue, uint8_t dir)
{
	...
	if(pFIO != NULL) {
		...
		// Input direction
			...
			// Upper
			if(halfwordNum) {
				pFIO->FIODIRU &= (uint16_t)(~bitValue);
			}
			// lower
			else {
				pFIO->FIODIRL &= (uint16_t)(~bitValue);
			}
		}
	}
}
...
void FIO_HalfWordSetMask(uint8_t portNum, uint8_t halfwordNum, uint16_t bitValue, uint8_t maskValue)
{
	...
	if(pFIO != NULL) {
		...
		// Un-mask
			...
			// Upper
			if(halfwordNum) {
				pFIO->FIOMASKU &= (uint16_t)(~bitValue);
			}
			// lower
			else {
				pFIO->FIOMASKL &= (uint16_t)(~bitValue);
			}
		}
	}
}
...
void FIO_ByteSetDir(uint8_t portNum, uint8_t byteNum, uint8_t bitValue, uint8_t dir)
{
	...
	if(pFIO != NULL) {
		...
		// Input direction
			...
			if (byteNum <= 3) {
				pFIO->FIODIR[byteNum] &= (uint8_t)(~bitValue);
			}
		}
	}
}
...
void FIO_ByteSetMask(uint8_t portNum, uint8_t byteNum, uint8_t bitValue, uint8_t maskValue)
{
	...
	if(pFIO != NULL) {
		...
		// Un-mask
			...
			if (byteNum <= 3) {
				pFIO->FIOMASK[byteNum] &= (uint8_t)(~bitValue);
			}
		}
	}
}
